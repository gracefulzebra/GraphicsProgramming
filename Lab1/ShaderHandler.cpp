...
void ShaderHandler::InitializeGeomShader(const std::string& filename)
{
	program = glCreateProgram(); // create shader program (openGL saves as ref number)
	shaders[0] = CreateShader(LoadShaderFromFile(filename + ".vert"), GL_VERTEX_SHADER); // create vertex shader
	shaders[1] = CreateShader(LoadShaderFromFile(filename + ".geom"), GL_GEOMETRY_SHADER); // create geometry shader
	shaders[2] = CreateShader(LoadShaderFromFile(filename + ".frag"), GL_FRAGMENT_SHADER); // create fragment shader
